...
const Quiz = sequelize.define(
  // define Quiz model (table quizzes)
  'quiz',
  {
...
(async () => {
  // IIFE - Immediatedly Invoked Function Expresión
  try {
...
const indexView = (quizzes) =>
  ...
  quizzes
    ...
    .join('\n') +
  `</div></table><a href="/quizzes/new" class="button">New Quiz</a>
    </body>
    </html>`;
...
const commonPart = (quiz) => {
  return `<input type="text" name="question" value="${quiz.question}" placeholder="Question">
...
const checkController = async (req, res, next) => {
  ...
  try {
    ...
    let msg =
      quiz.answer.toLowerCase().trim() === response.toLowerCase().trim()
